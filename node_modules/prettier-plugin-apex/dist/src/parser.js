...
export default async function parse(sourceCode, options) {
    ...
    if (options.apexStandaloneParser === "built-in") {
        ...
        serializedAst = await parseTextWithSpawn(serializerBin, sourceCode, options.parser === "apex-anonymous");
    }
    else {
        serializedAst = await parseTextWithSpawn(path.join(await getSerializerBinDirectory(), `apex-ast-serializer${process.platform === "win32" ? ".bat" : ""}`), sourceCode, options.parser === "apex-anonymous");
    }
    if (serializedAst) {
