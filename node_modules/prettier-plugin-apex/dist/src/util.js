...
export function isBinaryish(node) {
    return (node["@class"] === apexTypes.BOOLEAN_EXPRESSION ||
 ...
 */
export function isApexDocComment(comment) {
...
export function checkIfParentIsDottedExpression(path) {
    ...
    else if (node["@class"] === apexTypes.VARIABLE_EXPRESSION &&
 ...
 */
export function massageAstNode(ast, newObj) {
    // Handling ApexDoc
    if (ast["@class"] &&
    ...
    if (ast.scope && typeof ast.scope === "string") {
        ...
        for (let i = newObj.expr.length - 1; i >= 0; i -= 1) {
            if (newObj.expr[i]["@class"] === apexTypes.WHERE_COMPOUND_EXPRESSION &&
 ...
 */
export function findNextUncommentedCharacter(sourceCode, character, fromIndex, commentNodes, backwards = false) {
...
export async function getSerializerBinDirectory() {
    let serializerBin = nodePath.join(url.fileURLToPath(new URL(".", import.meta.url)), "../vendor/apex-ast-serializer/bin");
    /* v8 ignore start */
    if (!(await doesFileExist(serializerBin))) {
        serializerBin = nodePath.join(url.fileURLToPath(new URL(".", import.meta.url)), "../../vendor/apex-ast-serializer/bin");
    }
    /* v8 ignore stop */
